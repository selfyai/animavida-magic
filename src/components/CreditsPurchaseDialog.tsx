...
export function CreditsPurchaseDialog({ open, onOpenChange, onPurchaseComplete }: CreditsPurchaseDialogProps) {
  ...
  const handlePurchase = async () => {
    ...
    try {
      const { data: functionData, error: functionError } = await supabase.functions.invoke('create-pix-payment', {
  ...
  const startPaymentCheck = async (paymentId: string) => {
    ...
    const checkInterval = setInterval(async () => {
      ...
      try {
        ...
        const { data: checkData, error: checkError } = await supabase.functions.invoke('check-pix-payment', {
          body: { 
            paymentId, 
            userId: user.id, 
