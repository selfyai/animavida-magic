...
const DEFAULT_CONTENT: HomePageContent = {
  ...
  features: [{
    ...
    emoji: '🎤',
    title: 'Voz',
    description: 'Escolha a voz perfeita'
  }, {
    emoji: '✍️',
    title: 'Texto',
    description: 'Digite o que falar'
  }, {
    emoji: '🎬',
    title: 'Pronto!',
    description: 'Vídeo em segundos'
  }],
...
const Index = () => {
  ...
  const loadHomeContent = async () => {
    try {
      const {
        data
      } = await supabase.from('app_settings').select('value').eq('key', 'home_page_content').maybeSingle();
      if (data?.value) {
  ...
  return <div className="min-h-screen bg-background pb-safe-mobile">
      ...
      <main className="max-w-md mx-auto px-4 pt-2 pt-pwa-adjusted md:pt-8 pb-4">
        <div className="text-center mb-4 md:mb-8">
          <img src={logoUrl || logo} alt="Logo" className="h-10 mx-auto mb-3" onError={e => {
        ...
        <div className="grid grid-cols-2 gap-4 mb-6 md:mb-8">
          {homeContent.features.map((feature, index) => <div key={index} className="p-4 rounded-2xl bg-gradient-card border border-border">
        ...
        {!loading && <div className="text-center space-y-4">
            {isAuthenticated ? <div className="p-6 rounded-2xl bg-gradient-primary shadow-glow">
                <p className="font-medium mb-2 text-background">
                  Toque no ícone da câmera abaixo
                </p>
                <p className="text-sm text-background font-light">
